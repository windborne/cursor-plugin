# Weather by WindBorne for Cursor and Grok Bot

Ask Cursor or Grok Bot about the weather and get WindBorne's forecast back
as interactive weather apps: cards the MCP server returns alongside each
...
on them. This plugin connects Cursor and Grok Bot to WindBorne's hosted MCP
...
PKCE. The `CLIENT_ID` in `.mcp.json` is a public identifier, not a secret;
no API key or secret is needed or stored in this repository. Tokens are
held by the client: on your machine for Cursor desktop, on Cursor's side
for Cursor web and Grok Bot.
